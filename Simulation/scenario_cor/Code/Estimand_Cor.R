...
############################################
# This file computes the causal estimands under the IPS policy with correctly
...
for(simul.id in 1:D){
  ...
  estimands.list <- lapply(N, estimand.cor, deltas = deltas, r = r)
  ...
  ### Save output ###
  saveRDS(estimands, file = paste0("Simulation/scenario_cor/Data/estimand_id", simul.id,".rds"))
}
