...
############################################
# This file estimates the causal estimands under the IPS policy with mis-specified
...
for(sim.id in 1:D){
  ...
  for(s in 1:S){
    ...
    result <- estimator.mis(dat, X.trt, X.out, deltas, K, r)
  ...
  ## Save output
  saveRDS(est.par, file = paste0("Simulation/scenario_mis/Data/estimate.par_id", sim.id,".rds"))
  saveRDS(se.par,  file = paste0("Simulation/scenario_mis/Data/se.par_id", sim.id,".rds"))
  
  saveRDS(est.nonpar, file = paste0("Simulation/scenario_mis/Data/estimate.nonpar_id", sim.id,".rds"))
  saveRDS(se.nonpar,  file = paste0("Simulation/scenario_mis/Data/se.nonpar_id", sim.id,".rds"))
