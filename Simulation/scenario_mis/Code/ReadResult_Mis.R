...
############################################
# This file requires simulated estimands and estimates under "Simulation/scenario_mis/Data/".
...
###------ Read estimand ------###
file.list <- list.files("Simulation/scenario_mis/Data", pattern = "estimand.*rds")
D <- length(file.list)
...
for(file in file.list){
  estimands <- estimands + readRDS(paste0("Simulation/scenario_mis/Data/", file))
}
...
est.par.file.list <- list.files("Simulation/scenario_mis/Data/", pattern = "estimate.par.*rds")
se.par.file.list <- list.files("Simulation/scenario_mis/Data/", pattern = "se.par.*rds")

est.nonpar.file.list <- list.files("Simulation/scenario_mis/Data/", pattern = "estimate.nonpar.*rds")
se.nonpar.file.list <- list.files("Simulation/scenario_mis/Data/", pattern = "se.nonpar.*rds")
...
for(i in 1:D){
  ...
  if(i == 1){
    est.par.list = bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", est.par.file)))
    se.par.list  = bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", se.par.file)))
    est.nonpar.list = bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", est.nonpar.file)))
    se.nonpar.list  = bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", se.nonpar.file)))
  }else{
    est.par.list = rbind(est.par.list, bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", est.par.file))))
    se.par.list  = rbind(se.par.list,  bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", se.par.file))))
    est.nonpar.list = rbind(est.nonpar.list, bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", est.nonpar.file))))
    se.nonpar.list  = rbind(se.nonpar.list,  bind_rows(readRDS(paste0("Simulation/scenario_mis/Data/", se.nonpar.file))))
  }
}
...
for(delta.idx in seq_len(length(deltas))){
  ...
  write.csv(x = result.table, file = paste0("Simulation/scenario_mis/Result/SimulResult_Mis_delta_",delta,".csv"))
